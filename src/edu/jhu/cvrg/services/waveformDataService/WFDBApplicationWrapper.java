...
//			int iIndexLastSlash = sHeaderFile.lastIndexOf("/");
			int iIndexPeriod = sHeaderFile.lastIndexOf(".");
//			String sPath = sHeaderFile.substring(0,iIndexLastSlash);
...
//				sCommand = "rdann -a qrs -r " + sRecord + " > " + sRecord + "_sqrs.txt";
//				bRet = executeCommand(sCommand, asEnvVar, sPath);
//				stdReturnHandler();
//				bRet &= stdErrorHandler();
//				// set second output filename to the text file generated by rdann command.
//				outputFilename2 = sPath + "/" + sRecord + "_sqrs.txt";
			}else{
...
//			bRet &= 
			...
			if(bRet){
				...
				//outputFilenames = WFDBApplicationWrapper.empty_Strings;
				debugPrintln("- Encountered errors.");
			}			
	...
	public boolean wrsamp(String sInputFile, String sPath, 
		...
		//debugPrintln("- sFileName:" + sFileName);  
...
//			if(dMultiply > 0) sCommand += " -v " + dMultiply;
			if(bNoZero) sCommand += " -z";

			/*if (bCheckInput)
			{
				sOutputFile += sRecord + ".csv";
			}
			else if (bXML)
			{
				sOutputFile += sRecord + ".xml";
			}
			else
			{
				sOutputFile += sRecord + ".txt";
			}			
            */
			// essentially, we will be trying to stream the data and write it line by line to the output file as we receive it
			...
			if (bCheckInput)
			{
				stdReturnHandler(sPath + "/" + sRecord + ".csv");  // -c switch
			}
			else if (bXML)
			{
				stdReturnHandler(sPath + "/" + sRecord + ".xml");  // make an if-else
			}
			else
			{
				stdReturnHandler(sPath + "/" + sRecord + ".txt");  // make an if-else
			}
				*/
	...
	public boolean tach(String sInputFile, String sPath, 
		...
		{
			sOutputName = sRecord;
			//int iIndexPeriod2 = sInputFile.lastIndexOf(".");
			//sOutputName = sInputFile.substring(0, iIndexPeriod2);
		}
	...
	public boolean ann2rr(String sInputFile, String sPath, 
		...
		//String sCommand = "ann2rr -r " + sRecord + " -a " + sAnnotator + " -A -i 58 -w | /opt/pNNX/pnnlist"; // record name
...
//	private String inputFilename1 = "dummyInput1.txt";
//	private String inputFilename2 = "dummyInput2.txt";
	...
	private void debugPrint(String text){
		if(verbose)	System.out.print(text);
	}
